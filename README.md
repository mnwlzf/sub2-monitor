...
Priority 同步和自动调度开关通过 Sub2API Admin API 执行，避免绕过 Sub2API 缓存：

```yaml
sub2api:
  admin_base_url: "https://your-sub2api.example.com"
  admin_api_key: "admin-change-this-key"
  auto_schedulable_enabled: false
  auto_schedulable_failure_threshold: 3

  # 可选：只读连通性检查和历史 SQL 工具使用，不参与 Priority 同步。
  database:
...
账号 Priority 同步会通过 `POST /api/v1/admin/accounts/bulk-update` 更新 `priority`，
Admin API 鉴权使用 `x-api-key`。连续预采集失败自动暂停/恢复账号时也使用同一 Admin API。
...
- Sub2API Admin API Priority 同步和自动 schedulable 暂停/恢复
